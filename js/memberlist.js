var membersApp = angular.module('membersApp', ['ngAnimate']);

membersApp.controller('membersCtrl', ['$scope', '$timeout',
    function( $scope, $timeout ) {
        ...
        $scope.gotoPage = function(page) {
           ...
           $scope.showLoading = true;
            ...
            $.post(nectarLove.ajaxurl, data, function( response ){
               ...
               $scope.dopagearray();
               $timeout ( function() {
                   $('#members').animate( { opacity: 1 } ) 
               });
               $scope.showLoading = false;
            });
            $('#members').animate( { opacity: 0 } );
        };
        ...
        $scope.members = _members; // pushed from the PHP
        $scope.data = _data;
